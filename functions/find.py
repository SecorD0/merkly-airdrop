        ...
        for address_list in split_list(s_list=specified_addresses, n=100):
            for address_instance in address_list:
                ...
                if eligible_address_instance:
                    address_instance.points = eligible_address_instance.points
                    total_points += eligible_address_instance.points

                print(f'{n}\t{address_instance.address}\t{address_instance.points}')
                n += 1

            db.commit()

        eligible_addresses = db.all(Address, Address.points != 0)
        average_points = round(total_points / len(eligible_addresses)) if eligible_addresses else 0
        print(f'''
...
Total points: {config.LIGHTGREEN_EX}{format_number(total_points)}{config.RESET_ALL}
